...
func (s *ServiceImpl) GetDepartments() (map[string]Department, error) {
	...
	for _, team := range teams.Teams {
		...
		departments[team.Name] = Department{
			DisplayName: team.DisplayName,
			Physicians: lo.Map(physicians, func(p models.User, _ int) Employee {
				return Employee{
					ID:       p.ID,
					FullName: fmt.Sprintf("Doctor %s %s, MD", p.Firstname, p.Lastname),
				}
			}),
			Residents: lo.Map(residents, func(r models.User, _ int) Employee {
				return Employee{
					ID:       r.ID,
					FullName: fmt.Sprintf("Doctor %s %s, MD", r.Firstname, r.Lastname),
				}
			}),
			Nurses: lo.Map(nurses, func(n models.User, _ int) Employee {
				return Employee{
					ID:       n.ID,
